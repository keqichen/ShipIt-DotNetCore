...
namespace ShipItTest
{
    public class StockControllerTests : AbstractBaseTest
    {
        ...
        public void TestAddNewStock()
        {
            ...
            stockRepository.AddStock(1, new List<StockAlteration>(){new StockAlteration(productId, 1, 590)});
        ...
        public void TestUpdateExistingStock()
        {
            onSetUp();
            var productId = productRepository.GetProductByGtin(GTIN).Id;
            stockRepository.AddStock(1, new List<StockAlteration>() { new StockAlteration(productId, 2, 324) });

            stockRepository.AddStock(1, new List<StockAlteration>() { new StockAlteration(productId, 5,45) });
